...
class VideoAnnotationPathEditor(object):
	...
	def __setInterpolationType(self):
		...
		index = 0
		if self._interpolationMode=='slinear': 		index=1
		if self._interpolationMode=='quadratic': 	index=2
		if self._interpolationMode=='cubic': 		index=3

		item, ok = QtGui.QInputDialog.getItem(self, "QInputDialog.getItem()",
				"Interpolation mode:", items, index, False)
		if ok and item:
			if item==items[0]: self._interpolationMode=None
			if item==items[1]: self._interpolationMode='slinear'
			if item==items[2]: self._interpolationMode='quadratic'
			if item==items[3]: self._interpolationMode='cubic'
